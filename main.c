 ...
 *
 *
 *Questo programma gestisce il caricamento e il dowmload delle risorse presenti su una piattaforma didattica.
 *
 ...
 *ID (codice univoco), numero download totali (relativo alle risorse).
 *
 *Le risorse della piattaforma sono descritte da: titolo, livello educativo di riferimento, disciplina, parole
...
int main (void) {
	...
	if ((file_utenti = fopen("file_utenti.dat", "rb+")) == NULL)
		puts("\n\n\n\n\t\t\t\t\t File utenti non trovato!");

	if ((file_risorse = fopen("file_risorse.dat", "rb+")) == NULL)
		puts("\n\n\n\n\t\t\t\t\t File risorse non trovato!");

	if ((file_attivita = fopen("file_attivita.dat", "rb+")) == NULL)
		puts("\n\n\n\n\t\t\t\t\t File attivita' non trovato!");
	...
	if ((file_utenti != NULL) && (file_risorse != NULL) && (file_attivita != NULL))
	{
		...
		if (!fclose(file_utenti))
			puts("\n\n\n\n\t\t\t\t\t File utenti chiuso con successo.");

		if (!fclose(file_risorse))
			puts("\n\n\n\t\t\t\t\t File risorse chiuso con successo.");

		if (!fclose(file_attivita))
			puts("\n\n\n\t\t\t\t\t File attivita chiuso con successo.");
