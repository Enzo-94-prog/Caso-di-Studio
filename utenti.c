...
void apri_menu_creatore (FILE *file_utenti, FILE *file_risorse)
{
	...
	do {
		...
		puts("\t\t\t       *\t     4. Visualizza risorse creatore.      *");
 ...
 * @post L'utente puo' continuare il download di altre risorse o terminare l'operazione.
 */
void incrementa_numero_download_totali_utenti (FILE *file_utenti, int ID_creatore)
{
	...
	/*
	 * Questa procedura ha il compito di incrementare di un'unità il numero di download eseguiti
	 * dall'utilizzatore che ha appena scaricato una risorsa e il numero di volte che le risorse del creatore
...
void salva_attivita_utilizzatore (FILE *file_utenti, FILE *file_risorse, FILE *file_attivita, int ID_risorsa)
{
	...
	strcpy(attivita.titolo_risorsa, risorsa.titolo);               // titolo della risorsa
	strcpy(attivita.username_creatore, risorsa.username_creatore); // username del creatore
	strcpy(attivita.username_utilizzatore, utente.username);       // username dell'utilizzatore
	strcpy(attivita.disciplina, risorsa.disciplina);
	...
	attivita.valutazione = acquisisci_scelta(numero_massimo_stelle);

	localizza_spazio_libero_file_attivita (file_attivita);

	fwrite(&attivita, sizeof(attivita_t), 1, file_attivita);
}

/**
 * @pre L'utente deve aver scelto di visualizzare un elenco di utenti.
 * @post L'utente puo' visualizzare dettagliatamente i dati degli utenti stampati.
 */
void stampa_utente_singolo (utente_t *utente)
{
	...
	if (utente->tipo_account == 1) {
		...
		printf("\n\t\t\t\t  Le sue risorse sono state scaricate %d volte.\n\n", utente->numero_download_totali);
	}

	else if (utente->tipo_account == 0)
		printf("\n\t\t\t\t  L'utente ha scaricato risorse per %d volte.\n\n", utente->numero_download_totali);
 ...
 * in base al tipo di risorse da essi caricati.
...
/**
 * @pre L'utilizzatore deve aver scelto dal menu utilizzatore di visualizzare una lista dei 5 utenti piu' popolari,
...
void visualizza_utenti_piu_popolari (FILE *file_utenti, int tipo_account)
{
	...
	/*
	 * Se possibile, il file ordinamento viene riempito con tutte le strutture utente presenti all'interno
	 * del file utenti, al fine di eseguirne l'ordinamento.
	 */
