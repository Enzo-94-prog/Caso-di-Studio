...
/**
 * @pre Il creatore deve aver selezionato dal menu creatore l'opzione per aggiungere/modificare una delle sue risorse.
...
void aggiorna_valutazione_media_risorsa (FILE *file_risorse, FILE *file_attivita, int ID_risorsa)
{
	...
	while (!feof(file_attivita)) {
		...
		if ((strcmp(risorsa.titolo, attivita.titolo_risorsa) == 0)) {

			risorsa.valutazione_media += attivita.valutazione;
 ...
 * @post La struttura risorsa contenente i dati della risorsa e' stata scritta nel file risorse.
 */
...
/**
 * @pre Il creatore deve aver selezionato dal menu creatore l'opzione per cancellare una delle sue risorse.
 * @post Dal file risorse e' stata rimossa la struttura corrispondente alla risorsa scelta dal proprio creatore.
 */
...
/**
 * @pre Il creatore deve aver selezionato dal menu creatore l'opzione per modificare una delle sue risorse.
 ...
 * @post Sono state stampata a schermo tutte le eventuali risorse associate alla parola chiave immessa dall'utilizzatore.
 */
...
/**
 * @pre La struttura risorsa deve aver acquisito i dati di una risorsa presente all'interno del file risorse.
 ...
 * @post Se l'utente ha in precedenza scaricato almeno una risorsa vengono stampate una serie di risorse affini ai suoi gusti.
 */
void suggerisci_risorse (FILE *file_utenti, FILE *file_risorse, FILE *file_attivita)
{
	...
	/*
	 * Ammesso che l'utilizzatore abbia scaricato almeno una risorsa in precedenza, allora potrà farsi
	 * suggerire dal sistema tutte le risorse della affini alla disciplina della prima risorsa scaricata.
	 */
	...
	while ((!feof(file_attivita)) && (attivita_trovata == 0)) {
		...
		if ((strcmp(utente.username, attivita.username_utilizzatore) == 0)) // l'utente ha effettuato un download in passato
			attivita_trovata = 1;
	...
	if (attivita_trovata == 1) {
		...
		while (!feof(file_risorse)) {
			...
			if ((strcmp(risorsa.disciplina, attivita.disciplina) == 0))
			{
		...
		if (numero_risorse_trovate > 0) {
			...
			printf("\n\n\t\t    Vuoi scaricare una delle risorse suggerite? Digitare 1 in caso affermativo, altrimenti 0.");
			...
			do {
				...
				if (scelta == 1) {
					...
					if (numero_risorse_trovate > 1) {
						...
						puts("\n\n\t\t\t     Si vuole scaricare un'altra delle risorse suggerite? Digitare 1 in caso affermativo, altrimenti 0.");
		...
		puts("\n\n\t\t   Non e' stato possibile suggerire delle risorse affini a quelle gia' scaricate in precedenza dall'utente.\n");
...
/**
 * @pre Il creatore deve aver selezionato dal menu creatore l'opzione per visualizzare tutte le sue risorse.
 * @post Vengono stampate tutte le risorse aggiunte dal creatore.
 */
void visualizza_risorse_creatore (FILE *file_risorse)
{
	...
	if (numero_risorse_aggiunte == 0)
		...
		puts("\n\n\n\n\t\t\t\t      ** Tutte le risorse sono state stampate. **\n\n");
...
/**
 * @pre L'utilizzatore deve aver selezionato dal menu utilizzatore l'opzione per visualizzare le risorse piu' scaricate della piattaforma.
 * @post Vengono stampate le prime 5 risorse piu' scaricate della piattaforma.
 */
void visualizza_risorse_piu_scaricate (FILE *file_risorse)
{
	...
	/*
	 * Se possibile, il file ordinamento viene riempito con tutte le strutture risorsa presenti all'interno
	 * del file risorse, al fine di eseguirne l'ordinamento.
	 */


	if ((file_ordinamento = fopen("file_ordinamento.dat", "wb+")) == NULL)
		puts("\n\n\n\n\t\t\t\t\t File ordinamento non trovato!");
		...
		puts("\n");
			...
			puts("\n\n\t\t       ** Non vi sono risorse all'interno della piattaforma. Impossibile eseguirne l'ordinamento **");
	...
	if (!fclose(file_ordinamento))
		puts("\n\n\n\n\t\t\t\t\t File ordinamento chiuso con successo.");
...
int verifica_esistenza_risorsa (FILE *file_risorse)
{
	...
	/*
	 * Dopo che l'utente ha immesso un titolo, si controlla se quest'ultimo è già associato ad una risorsa
