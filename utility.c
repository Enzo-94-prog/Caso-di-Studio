...
int acquisisci_ID (void)
{
	 ...
	 * Dopodiché, essendo la sua ultima cifra salvata all'interno dell'ultimo
	 * elemento del vettore, si pone i uguale alla lunghezza del numero, sottraendogli
	 * 1, cosicché numero[i] sia uguale alla cifra stessa.
	 *
	 * Si avvia un while che si interromperà quando il valore della i sarà minore di 0,
	 ...
	 * del vettore numero (diminuita di 48 per risalire al suo valore numerico, anziché di carattere,
	 * in quanto è stata acquisita come stringa), moltiplicata per 1. Se in seguito a eventuali cicli
	 * successivi al primo la i sarà ancora maggiore di 0, la variabile moltiplicatore si decuplicherà
	 ...
	 *
	 * Se, ad esempio, l'utente ha immesso il numero 7, la i varrà 0 all'inizio (strlen (che è 1) - 1 == 0,
	 * in quanto è un'unica cifra). Una volta entrati nel ciclo, numero_finale vedrà il suo valore (pari a 0,
	 * all'inizio) sommato a quello dell'ultima cifra del numero immesso dall'utente (dopo che è stato diminuito
	 * di 48), e moltiplicato per 1, poiché è appunto l'ultima cifra. i verrà decrementato, il while si
	 * interromperà, e il numero_finale varrà 7. Questo valore verrà ritornato all'utente e corrisponderà, eventualmente
	 * al numero da lui immesso inizialmente.
	 * Lo scopo di tutto questo procedimento è assicurarsi che l'utente immetta solo valori validi, dato che
	 * la scanf tradizionale, acquisendo una serie di cifre preceduta però da anche un solo carattere, tende
	 ...
	 * come caratteri) e così via...
	 * Per cui, avendo acquisito quello che per noi è un numero, ma come carattere (il placeholder %s nella scanf
	 * ce lo indica), al fine di risalire al suo valore numerico, è sufficiente sottrargli 48.
	 */
 ...
 * @post Sara' stata individuata la prima posizione libera all'interno del file attivita'.
 */
void localizza_spazio_libero_file_attivita (FILE *file_attivita)
{
	...
	/*
	 * Il puntatore al file attività viene posizionato sul primo punto libero di questi.
	 */
	...
	while (fread(&attivita, sizeof(attivita_t), 1, file_attivita) != 0);
}
...
void svuota_buffer_input (void)
{
	 ...
	 * i caratteri eventualmente rimasti al suo interno, così da evitare effetti indesiderati sui successivi
	 * input.
	 */
	while (getchar() != '\n');
}
...
int acquisisci_scelta (int n_opzioni)
{
	...
	/*
	 * Questa funzione viene eseguita ogni qualvolta all'utente è richiesto di effettuare una scelta dal
	 * menu visualizzato al momento, immettendo un numero che le corrisponda. La scanf da questo punto
	 * di vista è problematica, in quanto immissioni "miste" come "1a" sono da essa riconosciute identiche
	 * a "1". Quindi la scelta dell'utente (che dev'essere numerica, alla fine) viene salvata all'interno
	 * di un vettore di caratteri di due elementi. Se viene individuato che il secondo non è vuoto, allora
	 * gli verrà richiesta una nuova immissione (perché la scelta corretta deve essere di UNA sola cifra).
	 * Il motivo per cui a scelta[0] viene sottratto 48 è analogo a quello illustrato nel commento
...
int controlla_credenziali_accesso (FILE *file_utenti, char *username, char *password, int ID, int *tipo_account)
{
	...
	/*
	 * Se nel file utenti viene individuata una struttura associata all'username ed alla password immessi
	 * dall'utente, la variabile account_trovata acquisirà il valore 1, così da segnalare alla funzione
	 * chiamante che l'account specificato dall'utente esiste, e che questi può effettuare l'accesso.
	 */
...
int controlla_scelta (char *scelta, int n_opzioni)
{
	...
	/*
	 * Viene comparato il valore di scelta[0], che è una cifra, con quello di n_opzioni, al fine di stabilire
	 * se la scelta effettuata dall'utente è fra quelle consentite.
	 * In caso affermativo, la variabile successo viene avvalorata ad 1. Infine, il suo valore è restituito
...
int controlla_username_registrazione (FILE *file_utenti, int ID)
{
	 ...
	 * del file utenti, allora la variabile username_libero viene avvalorata ad 1, e dunque restituita come
...
int genera_ID (char *stringa)
{
	 ...
	 * le strutture (utenti e risorse) all'interno dei file a loro corrispondenti.
	 ...
	 * alla sua posizione relativa, cioé rispetto agli altri caratteri.
	 */
